...
- ✅ **ルーム履歴** (v6.8新機能): 訪問したルームをプルダウンから選択可能
...
### 概要
訪問したルームを記憶し、ヘッダーのプルダウンから素早く移動できます。

### 使い方

#### 1. ルーム履歴の確認
ヘッダーの「ルーム」プルダウンをクリックすると、訪問履歴が表示されます。
...
#### 2. ルームの切り替え
プルダウンから別のルームを選択すると、そのルームに移動します。

#### 3. 新しいルームの作成
「＋ 新しいルームを作成...」を選択すると、ルーム名入力プロンプトが表示されます。
...
#### 4. 履歴の管理
- **自動保存**: 訪問したルームは自動的に履歴に追加
- **最大保存数**: 20件（古いものから自動削除）
- **ソート順**: 最終訪問日時の降順
- **保存場所**: ブラウザのlocalStorage（ブラウザごとに保存）
...
#### UIでモデルを選択
ヘッダーの「🤖 モデル」ドロップダウンから、使用するOllamaモデルを選択できます。

#### 特徴
- **ルームごと管理**: 各ルームで異なるモデルを使用可能
- **リアルタイム切り替え**: モデル変更は全参加者に即座に反映
- **システム通知**: モデル変更時に通知メッセージが表示

#### 使い方
1. ヘッダーの「🤖 モデル」ドロップダウンをクリック
2. インストール済みのモデルから選択
3. モデルが変更され、次回のAI編集から適用
...
├── server.js             # サーバー本体（v6.7対応版）
├── db.js                 # データベース操作
├── security.js           # セキュリティモジュール
├── data/                 # データベース保存先（自動生成）
│   └── rooms.db          # SQLiteデータベース
├── public/
│   └── index.html        # フロントエンド（v6.8対応版）
...
- **Temperature値**: 0.0-2.0の範囲に制限（v6.6機能）
...
**概要:**
訪問したルームを記憶し、ヘッダーのプルダウンから簡単に移動できる機能を追加しました。
...
**技術的な実装:**
- `RoomHistoryManager` クラスの追加
- localStorage スキーマ: `markdown-editor-room-history`
- 最終訪問日時でソート
- ページ遷移によるルーム切り替え
...
**バリデーション:**
- ルーム名の入力チェック（英数字、ハイフン、アンダースコア）
- 1-50文字の制限
- わかりやすいエラーメッセージ

### アップグレード手順

既存のv6.7からアップグレードする場合：
...
**注意事項:**
- サーバー側（server.js）の変更は不要
- 既存のデータベースはそのまま使用可能
- 既存の設定（localStorage）は保持される
...
*リリース日: 2025年10月1日*
*バージョン: 6.8*
*主な変更: ルーム履歴機能 + プルダウンUI + 新しいルーム作成機能*
