...
1. **AI指示の文字数制限撤廃**
   - security.jsから500文字制限を削除
   - マルチバイト文字での長文指示が可能に
   - より詳細で複雑な編集指示に対応

2. **AIモデルの自由選択機能**
   - UIからモデルを選択可能（ヘッダーのドロップダウン）
   - ルームごとに異なるモデルを使用可能
   - リアルタイムでモデル切り替え
   - システムメッセージでモデル変更を通知

3. **技術的な実装詳細**
   - server.js: モデル変更ハンドラ追加（606-656行目）
   - index.html: モデル選択UI有効化
   - ルームデータ構造にモデル情報追加
   - callOllama関数が動的なモデルパラメータに対応

### アップグレード手順

既存のv6.6からアップグレードする場合：
...
*リリース日: 2025年10月1日*
*バージョン: 6.7*
*主な変更: AI指示の文字数制限撤廃 + AIモデル自由選択機能 + ルームごとモデル管理*
